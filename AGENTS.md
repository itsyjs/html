...
- Oracles: the checks against outside sources live in a separate repository, `itsy-html-spec`, which expects this checkout beside it as `../@itsy+html` and reads it through its `lib` link. `pnpm test` there runs them against this source: the W3C ACT Rules examples, the tables in `src/a11y.ts` against aria-query, the audit against the tree parse5 builds (over the html5lib inputs, and over generated nestings in both directions), every finding's message and offset, and the injection oracle, which puts hostile values in every place a `${…}` can sit (`pnpm test:built` there runs it on this `dist/`). `pnpm mutate` there deletes each statement of `src/check.ts`, `audit.ts`, `a11y.ts` and `html.ts` in turn and fails on any deletion that this suite and the oracles both miss, unless its `mutants.ts` says why. Run both after changing those files.
