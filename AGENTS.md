...
- `src/audit.ts` and `src/a11y.ts` are dev-only and tree-shaken out of prod; `src/a11y.ts` is reached only from `check()`'s `__DEV__` branch, and `test/built/output.test.ts` scans the prod bundles to prove it never arrives there.
