...
| import path         | exports                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@itsy/html`        | `html`, `trusted`, `attrs`, `cx`, `raw`, `Html`, `isHtml`, `HtmlError`; types `Renderable`, `AttrValue`, `AttrGroup`, `ClassValue`, `StyleValue`                                                                                                                                                                                                                                                                                                                                                                |
...
html`<script>${raw(JSON.stringify(state).replace(/</g, '\\u003c'))}</script>`; // the only way into <script>; escape < manually or use devalue
...
- Inside a tag: only `attrs()`/`raw()` — and right after a `<` counts as inside a tag, since the browser reads a value there as the tag name. Inside `<script>`/`<style>`: only `raw()`, and a block with a `<!--` or `<![CDATA[` still open does not end at its end tag (SVG and the script escaped states read it as text). Inside `<!-- -->`: only `raw()` (a comment ends where the browser ends it: `-->`, `--!>`, and `<!-->`/`<!--->` are whole). After an unquoted value, or after a `${…}` inside a tag, a `=` starts a new attribute name, so a value after it is inside the tag. A list of those is fine.
...
- Dev build only, once per call site: the markup audit throws codes 8–14 — tag never closed with `>`, element still open at the end, end tag closing nothing/the wrong element, `<x />` on a non-void element, end tag on a void element, nesting the parser rewrites (`<a>` in `<a>`, `<div>` in `<p>`, `<tr>` in `<table>`, text directly in a table, an HTML tag inside SVG, anything after `</body>`, a table part outside a table, a second `<body>`), duplicate attribute. Every non-void element must be closed explicitly; the spec's omittable end tags (`<li>`, `<p>`, `<td>`…) are reported as 9/10. Deliberately unmatched tags go in `raw()`.
- `trusted` (root) is `html` without the escaping, for templates whose values are all the author's. Prod writes every value as it is: no escaping, no URL guard, no code 6. It still scans each template, so 3 and 5 hold. Dev is `html` plus code 20 for any value `html` would escape or block, so wherever dev renders, prod writes what `html` would. Its output nests into `html` unescaped, and `check()` cannot see it. Never use it for user data.
...
- Every `HtmlError` has a stable `code` (1–20, listed at https://itsyjs.github.io/html/reference/errors; 1 and 4 are reserved). The prod build's message is `E<code>`; the dev build (`development` export condition — Vite sets it in dev, esbuild/Node need `--conditions=development`) spells it out. Output is identical in both. For `trusted`, that holds because dev throws 20 wherever it would not.
...
- Oracles: the checks against outside sources live in a separate repository, `itsy-html-spec`, which expects this checkout beside it as `../@itsy+html` and reads it through its `lib` link. `pnpm test` there runs them against this source: the W3C ACT Rules examples, the tables in `src/a11y.ts` against aria-query, the audit against the tree parse5 builds (over the html5lib inputs, and over generated nestings in both directions), every finding's message and offset, the injection oracle, which puts hostile values in every place a `${…}` can sit, and the trusted oracle, which holds dev `trusted` to throwing 20 exactly where prod `trusted` and prod `html` would differ (`pnpm test:built` there runs both on this `dist/`). `pnpm mutate` there deletes each statement of `src/check.ts`, `audit.ts`, `a11y.ts`, `html.ts` and `trusted.ts` in turn and fails on any deletion that this suite and the oracles both miss, unless its `mutants.ts` says why. Run both after changing those files.
- Reviewing: a finding counts when it comes with a failing oracle case, or a new oracle. Anything else is a wishlist item, not a blocker. To change a rule, change what the oracle expects first.
...
- Whitespace is HTML's: tab, LF, FF, CR, space. Never `\s` in the scanner or the audit — it also matches a vertical tab or a no-break space, which the browser reads as part of a name or a value, so `<a\u00a0href="${x}">` puts `x` in the tag name.
