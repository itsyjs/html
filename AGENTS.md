...
| `@itsy/html/check`  | `check(markup, { ids? })` → `Problem[]` — the output validator; always `[]` in the prod build. Type `Problem`                                                                                                                                                                                                                              |
| `@itsy/html/a11y`   | `a11y`, `without(...rules)` — a rule set for `check(markup, { a11y })`. Nineteen accessibility rules, reported as `Finding` with a `rule` name instead of a numeric `code`. `without` silences rules by name, checked against `A11yRule`. Always inert in the prod build (29 B)                                                            |
...
- `check(view)` from `@itsy/html/check` runs the audit over a rendered string (cross-template) and adds 15 (dangling id reference), 16 (duplicate id) and 19 (a URL the guard blocked, `about:blank#blocked`). Returns a list; never throws.
