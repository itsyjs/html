...
// Renders one attribute onto `out` and hands back the new `out`. Called again for each
// key of an `aria` or `data` group. At module scope, not inside `createAttrs`: a closure
// there is a fresh allocation on every call, and this is the whole body of the work.
